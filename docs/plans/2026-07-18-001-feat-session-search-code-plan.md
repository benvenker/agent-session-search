...
1. **Where the recipe lives.** kimi: new `docs/recipes/code-mode-client-pattern.md`; codex: new `docs/code-mode.md`; fable: a section inside `docs/native-mcp.md`. **Decision: `docs/investigations/code-mode/recipe.md`** — amended 2026-07-18 per product-owner feedback: flat `docs/` pages document shipped features, and an experimental, unsupported recipe among them would imply a product surface this plan explicitly declines. The investigations folder already groups every code-mode artifact (digest, council report, revisit protocol); location is the loudest honest status signal. `docs/native-mcp.md` keeps one clearly-labeled experimental pointer for discoverability. (Supersedes the council's flat-page decision; the `docs/recipes/` taxonomy and native-mcp-section options remain rejected.)
...
- AE4. The packed npm tarball ships `docs/investigations/code-mode/recipe.md`, and its installed native command passes the same capabilities smoke the recipe describes.
...
- `docs/investigations/code-mode/recipe.md` — the experimental client-side composition recipe, plus inbound links.
...
| KTD6  | **Recipe lives at `docs/investigations/code-mode/recipe.md`; verdict lives in ADR 0002.** Flat `docs/` pages are shipped-feature documentation only; the experimental recipe lives with the code-mode investigation artifacts (product-owner amendment, disagreement resolution 1). ADR 0002 mirrors ADR 0001's role for a decision that must resist relitigation. (Disagreement resolutions 1 and 4.) |
...
- **Test scenarios:**
  ...
  - Packaging: tarball file list includes `docs/investigations/code-mode/recipe.md`; installed native binary lists capabilities and an approved tool (Covers AE4).
  - Docs contract: every command in the recipe is copy-paste runnable from repo root; the `docs/native-mcp.md` link resolves and is the only inbound pointer; `README.md`/`docs/README.md` do not link or present Code Mode (negative assertion); assertions fail on typed-schema, package-import, or parser-promising language (Covers AE3).
...
- **Files:** create `docs/investigations/code-mode/revisit-protocol.md`; modify `docs/investigations/code-mode/recipe.md` (one link).
...
### Direction clarifications (2026-07-18, post-synthesis, product owner)

Recorded after synthesis; these constrain Q1/bead-1 and any round-2 rerun without changing U1–U4.

1. **Code Mode's identity is an operation layer for the multiplex, never a ranker.** Legitimate scope: query construction, source selection, fanout orchestration, pagination, and mechanical namespace filtering. The model is the semantic ranker; FFF ranks match quality within each root; the aggregator stays deterministic. The round-1 prototype's failure was judgment smuggled into code (the lexical filter), not fanout — fanout, pagination, dedup, budgets, and ergonomics all passed. Any round 2 evaluates operation-layer value only; a reranker component invalidates the round.
2. **Envelope JSON = wrapper-side canonical metadata only.** The router knows every envelope field from dispatch, not from parsing FFF output: `source` id, canonical `root`, upstream `tool`, cursor/pagination state, byte counts, and budget snapshots. This is a _per-call_ envelope: it wraps the multiplex operation, and individual hit paths stay inside the opaque presentation-text payload — promoting per-hit paths to fields would require parsing (forbidden) or upstream structured output (Q1). These fields record _selected/dispatched_ provenance only — the router does not know, and the envelope must not claim, query relevance; relevance remains model judgment. Content remains opaque presentation text. **No timestamp field is promised**: a `date`/`mtime` field is deferred until its semantics are pinned (file mtime vs. session timestamp vs. path-derived date are different claims); namespace filtering ships on source/root plus caller-supplied path constraints alone.
3. **Source-namespace filtering is the motivating mechanical use case** (e.g. "Codex sessions" → drop non-`codex` results before model ranking). It also separates _matching_ (query text about a topic, all sources) from _filtering_ (harness namespace, one source) — impossible over raw text, trivial over a `source` field.
4. **Upstream-ask shaping.** FFF's design signals (frecency emitted as words, not scores; `trauncate_line_for_ai`; editorial size tags) suggest a deliberate model-as-ranker philosophy, so a typed-hits-with-scores ask risks principled rejection. The reduced ask, if envelope JSON proves insufficient, is mechanical per-hit metadata only — the seam exists (`crates/fff-mcp/src/output.rs` `OutputMode`; `rmcp`'s `structured_content` is unused at pinned commit 28321da / v0.9.6).
5. **Revisit-protocol demand signal, sharpened.** Watch for organic tasks where agents struggle to _operate_ the multiplex — multi-step joins (search → grep-within-result-paths), bulk sweeps with dedup, mechanical source/path post-filters — not for ranking dissatisfaction, which the managed lane's grouping + model selection already owns.
6. **Recipe location amended.** The recipe page lives at `docs/investigations/code-mode/recipe.md`, not flat `docs/code-mode.md`: flat docs pages document shipped features, and placement must not imply a product surface this plan declines. KTD6 and disagreement resolution 1 updated in place.
...
| Manual review    | Read `README.md`, `docs/native-mcp.md`, `docs/investigations/code-mode/recipe.md`, ADR 0002 as a new user | README shows no Code Mode surface; the single experimental pointer, trust boundary, SDK dependency, root-wide warning, and no-server-execution boundary are unambiguous. |
...
- `docs/investigations/code-mode/recipe.md` documents only the experimental client-side pattern and carries every mandatory warning, asserted by tests.
...
- The packed tarball ships `docs/investigations/code-mode/recipe.md`; the installed native command passes the capabilities smoke.
