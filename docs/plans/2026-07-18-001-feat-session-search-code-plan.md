...
3. **Launch instruction in the recipe.** kimi/fable: document `node dist/native-server.js` with the mode-664 workaround; codex: installed command only, never a repo `dist/` path. **Decision: installed command `agent-session-search-native-mcp` is the documented primary path**; the recipe carries one clearly-marked repo-development note for `node dist/native-server.js` naming the separately tracked mode-664/EACCES issue. Both audiences (installed users, repo developers) are real; codex's hard line would make the recipe unfollowable from a repo checkout, and kimi's framing would paper over a packaging defect as normal usage. The packaging test (U2) asserts the installed command launches — if it cannot, work stops and routes to the existing packaging bead rather than documenting around it (codex's order-changing question, adopted).
...
- Four follow-up Beads (filed, not implemented).
...
- Verification of the mode-664 executable-bit question from a packed artifact — linked onto the existing Session A packaging Bead, not a new competing one.
...
- Fixing the three separately tracked native-lane eval issues (bin modes 664, `shownLeadCount` shape, doctor `multi_grep` false-green) inside this work.
...
| KTD7 | **Installed command primary, repo fallback documented.** `agent-session-search-native-mcp` is the recipe's launch surface; `node dist/native-server.js` appears once as a repo-development note naming the tracked mode-664 issue. Packaging tests enforce the installed path. (Disagreement resolution 3.) |
...
flowchart TB
  ...
  T -->|usage / structured results / packaging fix| P[Fair battery in new throwaway worktree]
...
- **Approach:** File four beads (titles, not specs): (1) "Typed/structured native result contract — decision record needed (upstream-owned vs. wrapper-inferred)" — highest leverage; carries the tension with the two-lane plan's typed-schema deferral and must resolve whether raw presentation text is the permanent extraction ceiling; (2) "R12 gate rerun — fair managed-arm comparison per revisit-protocol.md" — blocked on triggers; (3) "Code Mode evidence watch — monthly organic native-lane usage check" — recurring-lite process bead carrying the observation queries and the dated ledger; (4) "Packed-install executable-bit verification for dist bins" — a **link** onto the existing Session A packaging bead, not a new competing bead.
- **Test scenarios:** Test expectation: none — issue-tracker operations.
- **Verification:** `br list` shows beads 1–3 with links to ADR 0002 and `docs/investigations/code-mode/revisit-protocol.md`; bead 4 links the existing packaging bead.
...
2. **Concurrency isolation:** Session A / Beads owns the three native-lane eval issues and will touch adjacent files (packaging, preflight, troubleshooting docs); a separate worktree prevents interleaved edits and lets U2 rebase over the executable-bit fix if it lands mid-stream.
...
2. **Revisit triggers (any one opens gate round 2):**
   ...
   - **T3 — packaging confirmed fixed:** the packed/installed artifact is verified executable, resolving the mode-664 question at the packaging layer.
...
| Scope creep into the three tracked eval issues | This work absorbs Session A's scope | They are referenced as prerequisites only; U4 bead 4 links rather than duplicates the packaging bead |
...
- **Q2: installed-command verification.** If `test/packaging.test.ts` shows the installed tarball cannot launch `agent-session-search-native-mcp`, stop U2 and route to the tracked packaging issue; do not ship a recipe whose primary path is broken.
...
- Four follow-up Beads exist with the stated links.
