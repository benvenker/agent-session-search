...
function App() {
  ...
  const collectQuery = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "collect",
    iteration: 0,
  });
  const writeQuery = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "write-skills",
    iteration: 0,
  });
  ...
  const collect = useMemo(
    () => extractCollect(collectQuery.data),
    [collectQuery.data]
  );
  const result = useMemo(
    () => extractWriteResult(writeQuery.data),
    [writeQuery.data]
  );
  ...
  const createdCount = result
    ? result.written.filter((w) => w.action === "created").length
    : 0;
  const updatedCount = result
    ? result.written.filter((w) => w.action === "updated").length
    : 0;
  const successCount = skillRuns.filter(
    (r) => statusClass(r.status) === "finished"
  ).length;
    ...
    <main className="shell" data-testid="workflow-skill-ui">
      ...
      <header className="topbar">
        <div className="title-group">
          ...
          {activeRun ? (
            <span
              className={"badge " + runStatus}
              data-testid="workflow-skill-status"
            >
          ...
          {activeRunId ? (
            <>
              <div className="inputbar" data-testid="workflow-skill-input">
                <div className="field">
                  ...
                  <span className="value">
                    {collect?.workflowTarget ?? workflow ?? "all"}
                  </span>
                </div>
                <div className="field">
                  <span className="label">Output</span>
                  <span className="value">
                    {collect?.output ?? collect?.defaultSkillDir ?? "default"}
                  </span>
                </div>
                <div className="field">
                  <span className="label">Extra prompt</span>
                  <span className="value">
                    {collect?.prompt && collect.prompt.length > 0
                      ? collect.prompt
                      : "none"}
                  </span>
                </div>
                ...
                {collectOpen ? (
                  collect ? (
                    <div style={{ marginTop: 12 }}>
                      ...
                      <ul
                        className="list"
                        data-testid="workflow-skill-workflows"
                      >
                      ...
                      <ul
                        className="list"
                        data-testid="workflow-skill-existing"
                      >
                    ...
                    <div
                      ...
                      data-testid="workflow-skill-collect-pending"
                    >
                ...
                {result ? (
                  <div style={{ marginTop: 12 }}>
                    <div
                      ...
                      data-testid="workflow-skill-summary"
                    >
                      {result.summary}
                    </div>
                    <ul className="list" data-testid="workflow-skill-written">
                      {result.written.map((w, i) => (
                        <li key={w.path + ":" + i}>
                          <span className="name">
                            {w.skillName || w.workflow}
                          </span>
                          <span className="mono">{w.path}</span>
                          <span className={"action-badge " + w.action}>
                            {w.action}
                          </span>
                        </li>
                      ))}
                      {result.written.length === 0 ? (
                        <li
                          className="empty-inline"
                          data-testid="workflow-skill-written-empty"
                        >
                  ...
                  <div
                    className="empty-inline"
                    data-testid="workflow-skill-results-pending"
                  >
                    {collectDone
              ...
              <div className="footer">
                ...
                {collectQuery.loading || writeQuery.loading ? (
            ...
            <div className="empty" data-testid="workflow-skill-empty">
              <div>
                Select a run from the sidebar, or launch a new one to generate
                skill documentation. No run selected.
              </div>
              <button
                className="button primary"
