...
function deriveGroups(runData: unknown): FeatureGroup[] {
  ...
  const input = isRecord(run.input)
    ? run.input
    : isRecord(run.inputs)
      ? run.inputs
      : {};
...
function extractGroupResult(value: unknown): GroupResult | null {
  ...
  const row = isRecord(response.row)
    ? response.row
    : isRecord(response)
      ? response
      : {};
...
function App() {
  ...
  const [selectedGroupIdx, setSelectedGroupIdx] = useState<number | undefined>(
    undefined
  );
  ...
  const mergeOutput = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "audit:merge",
    iteration: 0,
  });
  ...
  const mergeStarted =
    events.some(
      (e) =>
        ...
        /merge/.test(JSON.stringify(e.payload))
    ) || merge !== null;

  const activeGroupId =
    selectedGroupIdx !== undefined ? selectedGroupIdx : groups[0]?.index;
  ...
  async function copyReport() {
    const text =
      merge?.markdownBody ?? merge?.mergedResult ?? merge?.summary ?? "";
    try {
  ...
  const parallelActive =
    !parallelDone &&
    groupCount > 0 &&
    statusClass(activeRun?.status) === "running";
    ...
    <main className="shell" data-testid="audit-ui">
      ...
      <header className="topbar">
        <div className="title-group">
          ...
          {activeRun ? (
            <span
              className={"badge " + statusClass(activeRun.status)}
      ...
      <div className="phases" data-testid="audit-timeline">
        <div
          className={
            "phase " + (parallelDone ? "done" : parallelActive ? "active" : "")
          }
        >
          <span className="dot" />
        ...
        <div className="progress">
          <span
            style={{
              width: parallelDone ? "100%" : parallelActive ? "55%" : "0%",
            }}
          />
        </div>
        ...
        <div
          className={
            "phase " +
            (parallelDone
              ? statusClass(activeRun?.status) === "finished"
                ? "done"
                : "active"
              : "")
          }
              ...
              {merge ? (
                <>
                  <div className="card">
                    ...
                    <div data-testid="audit-report-summary">
                      {merge.summary ?? "(no summary)"}
                    </div>
                    <div className="meta-row">
                      <span>
                        {merge.totalGroups ?? groupCount} groups audited
                      </span>
                    </div>
                  </div>
                  <div className="card">
...
function GroupRow(props: {
  ...
  return (
    ...
    >
      ...
      <span className="meta">
        <span>
          {result?.featuresCovered.length ?? props.group.features.length}{" "}
          features
        </span>
        {result?.score !== undefined ? (
          <span className={"score " + scoreClass(result.score)}>
            {result.score}
          </span>
        ) : (
...
function GroupDetail(props: {
  ...
  const result = extractGroupResult(out.data);
  const features = result?.featuresCovered.length
    ? result.featuresCovered
    : props.group.features;
  return (
    <div className="card" data-testid="audit-group-card">
      <div className="detail-head">
        <h2 data-testid="audit-group-name">
          {result?.groupName ?? props.group.groupName}
        </h2>
        {result?.score !== undefined ? (
          <span
      ...
      {result?.result ? (
        <pre
          className="findings"
          style={{ marginTop: 14 }}
          data-testid="audit-group-findings"
        >
          {result.result}
        </pre>
      ) : (
        <div className="empty" style={{ padding: "32px 8px" }}>
          {props.runId
            ? "Waiting for this group's audit findings…"
            : "No run selected."}
        </div>
      )}
    </div>
