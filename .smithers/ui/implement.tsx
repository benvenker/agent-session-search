...
function App() {
  ...
  const activeRunId =
    selectedRunId ?? runIdFromUrl() ?? implementRuns[0]?.runId;
  ...
  const implDot = implement
    ? implement.allTestsPassing === false
      ? "err"
      : "ok"
    : "pending";
  const valDot = validate
    ? validate.allPassed === false
      ? "err"
      : "ok"
    : "pending";
          ...
          {hasAnyOutput ? (
            <>
              <div
                ...
                data-testid="implement-completion"
              >
                ...
                <div className="reviewers">
                  {reviews.map((r, i) => (
                    <div
                      className="reviewer-card"
                      key={i}
                      data-testid={"implement-reviewer-" + i}
                    >
                      <div className="reviewer-head">
                        <span className="reviewer-name">{r.reviewer}</span>
                        <span
                          className={
                            "verdict " + (r.approved ? "approved" : "rejected")
                          }
                        >
              ...
              {feedback ? (
                <div
                  ...
                  data-testid="implement-feedback"
                >
            ...
            <div className="empty" data-testid="implement-empty">
              <div className="empty-lead">
                Implement a code change with automated validation and code
                review. The workflow will iterate up to 3 times if validation or
                reviews fail, incorporating feedback each round.
              </div>
              <div>
                {activeRunId
                  ? "Waiting for the first task to run…"
                  : "No run yet."}
              </div>
              <button
                className="button primary"
