...
function extractImplement(value: unknown): ImplementOutput | null {
  ...
  return {
    summary,
    filesChanged,
    allTestsPassing: asBool(row.allTestsPassing),
  };
}
...
function App() {
  ...
  const [prompt, setPrompt] = useState(
    "Improve the test coverage for the current repository."
  );
  ...
  function stageEl(
    key: "implement" | "validate" | "review",
    label: string,
    done: boolean
  ) {
    const cls =
      "stage" + (done ? " done" : "") + (activeStage === key ? " active" : "");
    return (
    ...
    <main className="shell" data-testid="improve-test-coverage-ui">
      ...
      <header className="topbar">
        <div className="title-group">
          ...
          {activeRun ? (
            <span
              className={"badge " + runStatusClass}
              data-testid="improve-test-coverage-status"
            >
          ...
          {activeRunId ? (
            <span
              className="iter-pill"
              data-testid="improve-test-coverage-iteration"
            >
        ...
        <div className="toolbar">
          ...
          {activeRun &&
          (runStatusClass === "finished" || runStatusClass === "failed") ? (
            <button
              className="button"
      ...
      <div className="main">
        <div className="content">
          {activeRunId ? (
            <div
              className="timeline"
              data-testid="improve-test-coverage-timeline"
            >
          ...
          {hasAnyOutput ? (
            <>
              <div
                className="card full"
                data-testid="improve-test-coverage-implement"
              >
                <h2>
                  Implementation
                  {implement && implement.allTestsPassing !== undefined ? (
                    <span
                      className={
                        "badge " + (implement.allTestsPassing ? "ok" : "err")
                      }
                    >
                ...
                {implement ? (
                  <>
                    ...
                    {implement.filesChanged.length > 0 ? (
                      <ul
                        className="files"
                        data-testid="improve-test-coverage-files"
                      >
                      ...
                      <div
                        ...
                        data-testid="improve-test-coverage-files-empty"
                      >
                  ...
                  <div className="inline-empty">
                    Waiting for the implement stage…
                  </div>
                )}
              </div>
              ...
              <div className="panel-grid split">
                <div
                  className="card"
                  data-testid="improve-test-coverage-validate"
                >
                  <h2>
                    Validation
                    {validate && validate.allPassed !== undefined ? (
                      <span
                        className={
                          "badge " + (validate.allPassed ? "ok" : "err")
                        }
                      >
                  ...
                  {validate ? (
                    <>
                      ...
                      {validate.failingSummary ? (
                        <div
                          className="fail-box"
                          data-testid="improve-test-coverage-failing"
                        >
                          {validate.failingSummary}
                        </div>
                      ) : null}
                ...
                >
                  ...
                  {review ? (
                    <div className="reviewer">
                      ...
                      {review.feedback ? (
                        <div className="summary-text">{review.feedback}</div>
                      ) : null}
                      {review.issues.length > 0 ? (
                        review.issues.map((issue, i) => (
                          <div
                            className="issue-row"
                            key={i}
                            data-testid="improve-test-coverage-issue"
                          >
                            <span
                              className={"sev " + (issue.severity ?? "nit")}
                            >
                              {issue.severity ?? "nit"}
                            </span>
            ...
            <div className="empty" data-testid="improve-test-coverage-waiting">
              <div>
                Running iteration 1/3 — finding and adding missing tests…
              </div>
            </div>
          ) : (
            <div className="hero" data-testid="improve-test-coverage-empty">
              ...
              <p>
                Add high-impact missing tests for this repository. Start a run
                to find the gaps and verify the new tests pass.
              </p>
              <button
                className="button primary"
