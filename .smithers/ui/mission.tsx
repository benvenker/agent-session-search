...
function extractFinal(value: unknown): MissionFinal | null {
  ...
  return {
    ...
    summary,
    completedMilestones:
      typeof row.completedMilestones === "number" ? row.completedMilestones : 0,
    totalMilestones:
      typeof row.totalMilestones === "number" ? row.totalMilestones : 0,
...
function MilestoneDetail(props: {
  ...
  const integration = useMemo(
    () => extractIntegration(integrate.data),
    [integrate.data]
  );
  const validation = useMemo(
    () => extractValidation(validate.data),
    [validate.data]
  );
        ...
        {validation ? (
          <div className="feature" data-testid="mission-validation">
            <div className="ftitle">
              <span
                className={
                  "cstatus " + (validation.passed ? "passed" : "failed")
                }
              >
...
function App() {
  ...
  const planOutput = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "mission:plan",
    iteration: 0,
  });
  const approvalOutput = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "mission:approve-plan",
    iteration: 0,
  });
  const finalOutput = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "mission:final",
    iteration: 0,
  });

  const plan = useMemo(() => extractPlan(planOutput.data), [planOutput.data]);
  const approval = useMemo(
    () => extractApproval(approvalOutput.data),
    [approvalOutput.data]
  );
  const final = useMemo(
    () => extractFinal(finalOutput.data),
    [finalOutput.data]
  );
  ...
  const pendingApproval = useMemo(() => {
    ...
    return list.find(
      (a) => a.runId === activeRunId && a.nodeId === "mission:approve-plan"
    );
  }, [approvalsQuery.data, activeRunId]);

  const runStatus =
    (runDetail.data as RunSummary | undefined)?.status ?? activeRun?.status;
  ...
  const activeIndex = useMemo(() => {
    ...
    for (const ev of events) {
      ...
      const match = nodeId.match(
        /^mission:milestone:(\d+):(validate|revalidate)$/
      );
      ...
      <header className="topbar">
        <div className="title-group">
          ...
          <span className="pill" data-testid="mission-runid">
            <span className="mono">
              {hasAnyRun ? shortRunId(activeRunId) : "No run"}
            </span>
          </span>
          {hasAnyRun ? (
            <span
      ...
      <div className="main">
            ...
            <div className="launch-form" data-testid="mission-empty">
              ...
              <div className="field">
                ...
                <input
                  ...
                  value={maxMilestones}
                  onChange={(e) =>
                    setMaxMilestones(Number(e.currentTarget.value) || 1)
                  }
                />
              </div>
              <div className="field">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setMaxConcurrency(Number(e.currentTarget.value) || 1)
                  }
                />
              </div>
              <button
            ...
            <div data-testid="mission-plan-panel">
              ...
              {plan ? (
                ...
                <div className="card muted">
                  Waiting for the plan to be generated…
                </div>
              )}
              ...
              {pendingApproval ? (
                <div className="card approval" data-testid="mission-approval">
                  <h3>
                    {pendingApproval.requestTitle ?? "Approve mission plan?"}
                  </h3>
                  <div className="muted">
                    {pendingApproval.requestSummary ??
                      "Review the scoped plan before workers begin."}
                  </div>
                  <input
                ...
                <div className="card" data-testid="mission-approval-result">
                  <h3>
                    ...
                    <span
                      className={
                        "badge " + (approval.approved ? "finished" : "failed")
                      }
                    >
                  ...
                  {approval.note ? (
                    <div className="muted">{approval.note}</div>
                  ) : null}
                  {approval.decidedBy ? (
                    <div className="muted">by {approval.decidedBy}</div>
                  ) : null}
                </div>
              ) : null}
              ...
              {timelineMilestones.length > 0 ? (
                <ul className="timeline" data-testid="mission-timeline">
                  {timelineMilestones.map((m, i) => {
                    ...
                    const active =
                      i === activeIndex && statusClass(runStatus) === "running";
                    return (
                      <li
                        className="ms-row"
                        key={m.id}
                        data-testid={"mission-milestone-" + m.id}
                      >
                        <span
                          className={
                            "ms-dot" +
        ...
        <div className="col mid">
          {final ? (
            ...
            plan &&
            timelineMilestones.length > 0 &&
            activeIndex < timelineMilestones.length ? (
            <>
              <p className="section-head">Active milestone</p>
              <MilestoneDetail
                runId={activeRunId}
                index={activeIndex}
                milestone={timelineMilestones[activeIndex]}
              />
            </>
          ) : (
