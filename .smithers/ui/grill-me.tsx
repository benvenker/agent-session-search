...
function extractRow(value: unknown, iteration: number): GrillRow | null {
  ...
  return {
    ...
    questionsAsked:
      typeof row.questionsAsked === "number" ? row.questionsAsked : 0,
...
function App() {
  ...
  const [prompt, setPrompt] = useState(
    "Describe what you want to get grilled on."
  );
  ...
  const iter0 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: GRILL_NODE_ID,
    iteration: 0,
  });
  const iter1 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: GRILL_NODE_ID,
    iteration: 1,
  });
  const iter2 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: GRILL_NODE_ID,
    iteration: 2,
  });
  const iter3 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: GRILL_NODE_ID,
    iteration: 3,
  });
  const iter4 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: GRILL_NODE_ID,
    iteration: 4,
  });
  const iter5 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: GRILL_NODE_ID,
    iteration: 5,
  });
  const iter6 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: GRILL_NODE_ID,
    iteration: 6,
  });
  const iter7 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: GRILL_NODE_ID,
    iteration: 7,
  });
  ...
  const runStatus =
    (isRecord(runDetail.data) ? asString(runDetail.data.status) : undefined) ??
    activeRun?.status;
      ...
      <header className="topbar">
        <div className="title-group">
          ...
          <span className="pill" data-testid="grill-me-runid">
            <span className="mono">
              {hasRun ? shortRunId(activeRunId) : "No run"}
            </span>
          </span>
          {hasRun ? (
            <span
              className={"badge " + statusClass(runStatus)}
              data-testid="grill-me-status"
            >
        ...
        <section className="history" data-testid="grill-me-history">
          ...
          {rows.length > 0 ? (
            rows.map((row) => (
              ...
              >
                <div className="qa-meta">
                  ...
                  {row.branch ? (
                    <span className="branch-chip">{row.branch}</span>
                  ) : null}
                  {row.resolved ? (
                    <span className="badge finished">resolved</span>
                  ) : null}
                </div>
            ...
            <div className="empty" data-testid="grill-me-empty">
              ...
              <div>
                Launch a grill-me workflow from the Smithers CLI or dashboard.
              </div>
              <button
                className="button primary"
          ...
          <div className="rail-section">
            ...
            {sharedUnderstanding ? (
              <div
                ...
                data-testid="grill-me-understanding"
              >
                {sharedUnderstanding}
              </div>
            ) : (
              <div className="empty" data-testid="grill-me-understanding-empty">
                Not captured yet — the agent updates this as understanding
                builds.
              </div>
            )}
          </div>
          ...
          {resolved ? (
            <div className="rail-section">
              <div className="resolved-banner" data-testid="grill-me-resolved">
                ...
                <span>
                  {sharedUnderstanding ??
                    "The agent is satisfied with the requirements."}
                </span>
              </div>
            </div>
          ) : null}
