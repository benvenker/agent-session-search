...
function parseTicketNode(
  nodeId: string
): { slug: string; step: string; result: boolean } | null {
  ...
  if (!slug || slug === "tickets" || slug === "merge" || step === "loop")
    return null;
...
function deriveTickets(
  ...
  for (const event of events) {
    ...
    if (eventName === "node.failed") {
      ...
      existing.lane =
        existing.lane === "completed" ? existing.lane : "in-progress";
      existing.state =
        existing.state === "failed" || existing.state === "finished"
          ? existing.state
          : "in-progress";
  ...
  return Array.from(tickets.values()).sort((left, right) =>
...
function App() {
  ...
  const approvals = useGatewayApprovals({
    filter: { workflow: WORKFLOW_KEY, limit: 10 },
  });
  const actions = useGatewayActions();

  const kanbanRuns = useMemo(() => {
    return ((runs.data ?? []) as RunSummary[]).filter(
  ...
  const ticketsOutput = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "tickets",
    iteration: 0,
  });
  const streamEvents = useMemo(() => {
    return collectStreamEvents(
      stream.events as Array<Record<string, unknown>>
    ).filter((event) => !activeRunId || asString(event.runId) === activeRunId);
  }, [activeRunId, stream.events]);
  const discoveredTickets = useMemo(
    () => extractDiscoveredTickets(ticketsOutput.data),
    [ticketsOutput.data]
  );
  const tickets = useMemo(
    () => deriveTickets(discoveredTickets, streamEvents),
    [discoveredTickets, streamEvents]
  );
  const pendingApprovals = approvals.data ?? [];
  const ticketsByLane = useMemo(() => {
    const grouped: Record<BoardLane, TicketView[]> = {
      pending: [],
      "in-progress": [],
      completed: [],
    };
  ...
  async function decide(
    ...
    try {
      await actions.submitApproval({
        ...
        decision: {
          approved: ok,
          note: (ok ? "Approved" : "Denied") + " via UI",
        },
      });
                ...
                {ticketsByLane[lane].map((t) => (
                  <article className={"ticket " + t.state} key={t.slug}>
                    ...
                    <div className="ticket-meta">
                      ...
                      <div
                        className={
                          "pill " + (t.state === "in-progress" ? "active" : "")
                        }
                      >
                        {t.events} Events
                      </div>
            ...
            <div className="side-list">
              {kanbanRuns.map((r) => (
                <button
                  key={r.runId}
                  className={
                    "run-btn " + (r.runId === activeRunId ? "selected" : "")
                  }
                ...
                >
                  ...
                  <span className="run-time">
                    {formatTime(
                      asNumber(r.startedAtMs) ?? asNumber(r.createdAtMs)
                    )}
                  </span>
                </button>
              ))}
            ...
            <div className="side-list">
              {pendingApprovals.map((a) => (
                <div
                  ...
                  key={a.runId + a.nodeId + a.iteration}
                >
                  <div className="approval-txt">
                    {a.requestTitle ?? a.nodeId}
                  </div>
