...
function App() {
  ...
  const n0 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "review:0",
    iteration: 0,
  });
  const n1 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "review:1",
    iteration: 0,
  });
  const n2 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "review:2",
    iteration: 0,
  });
  const n3 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "review:3",
    iteration: 0,
  });
  const n4 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "review:4",
    iteration: 0,
  });
  const n5 = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "review:5",
    iteration: 0,
  });
  ...
  const visibleIssues =
    sevFilter === "all"
      ? allIssues
      : allIssues.filter((it) => it.severity === sevFilter);
    ...
    <main className="shell" data-testid="review-ui">
      ...
      <header className="topbar">
        <div className="title-group">
          <h1>Review</h1>
          <span className="pill" data-testid="review-runid">
        ...
        <div className="content">
          {synthesis !== null || reviews.length > 0 ? (
              ...
              <div className="lanes" data-testid="review-reviewers">
                {reviews.map((r) => (
                  <div
                    ...
                    className={
                      "lane " +
                      (r.approved ? "approved" : "blocked") +
                      (openLanes[r.index] ? " open" : "")
                    }
                    data-testid={"review-reviewer-" + r.index}
                    onClick={() =>
                      setOpenLanes((prev) => ({
                        ...prev,
                        [r.index]: !prev[r.index],
                      }))
                    }
                  >
                    <div className="lane-top">
                      ...
                      <span
                        className={
                          "badge " + (r.approved ? "finished" : "failed")
                        }
                      >
                        {r.approved ? "approved" : "denied"}
                      </span>
                    </div>
                    <div className="lane-feedback">
                      {r.feedback || "No feedback provided."}
                    </div>
                    <div className="lane-badges">
                      {SEVERITIES.map((s) => {
                        const c = r.issues.filter(
                          (it) => it.severity === s
                        ).length;
                        return c > 0 ? (
                          <span key={s} className={"sev " + s}>
                      ...
                      {r.issues.length === 0 ? (
                        <span className="sev nit">no issues</span>
                      ) : null}
                    </div>
                  </div>
                ))}
                ...
                {visibleIssues.map((it, i) => (
                  <div className="issue" key={i} data-testid="review-issue">
                    <div className="issue-top">
                      <span className={"sev " + it.severity}>
                        {it.severity}
                      </span>
                      <span className="issue-title">{it.title}</span>
                      {it.file ? (
                        <span className="issue-file">{it.file}</span>
                      ) : null}
                    </div>
                    {it.description ? (
            ...
            <div className="empty" data-testid="review-empty">
              ...
              <button
                className="button primary"
                data-testid="review-launch-empty"
                onClick={() => void launch()}
                disabled={busy}
              >
                Launch Review
            ...
            >
              ...
              {r.runId === activeRunId && reviews.length > 0 ? (
                <span className="approve-count">
                  {approvedCount}/{reviews.length} approved
                </span>
              ) : null}
            </button>
          ))}
