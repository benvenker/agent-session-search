...
function statusClass(status: string | undefined) {
  ...
  if (status === "waiting-approval" || status === "waiting-timer")
    return "running";
  return "";
}
...
function eventLabel(ev: unknown): { text: string; kind: string } {
  ...
  const message =
    asString(ev.message) ?? asString(ev.text) ?? asString(ev.chunk);
  const parts = [type];
...
function App() {
  ...
  const researchOutput = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "research",
    iteration: 0,
  });
  const research = useMemo(
    () => extractResearch(researchOutput.data),
    [researchOutput.data]
  );

  const runData = runQuery.data;
  const status =
    asString(isRecord(runData) ? runData.status : undefined) ??
  ...
  const { started, finished } = extractTimes(
    runData,
    activeSummary?.createdAtMs
  );
  ...
  async function rewind() {
    ...
    try {
      if (typeof actions.rewindRun === "function") {
        await actions.rewindRun({
          runId: activeRunId,
          frameNo: 0,
          confirm: true,
        });
      } else {
        await actions.launchRun({
          workflow: WORKFLOW_KEY,
          input: { prompt: queryPrompt ?? prompt },
        });
      }
      await refresh();
    } finally {
        ...
        <div className="content">
          {hasActiveRun ? (
            <>
              ...
              <section className="card" data-testid="research-query">
                ...
                <div className="query-text">
                  {queryPrompt ?? "(no prompt recorded for this run)"}
                </div>
                {errorMsg ? (
                  ...
                  {events.length === 0 ? (
                    ...
                    events.map((ev, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className={"stream-line" + (isTask ? " task" : "")}
                        >
                          {text}
                        </div>
                      );
              ...
              <section className="card" data-testid="research-summary">
                ...
                {research ? (
                  <>
                    ...
                    {research.keyFindings.length > 0 ? (
                      ...
                      <div
                        ...
                        data-testid="research-findings-empty"
                      >
                  ...
                  <div
                    className="findings-empty"
                    data-testid="research-summary-pending"
                  >
                    {isRunning(status)
            ...
            <div className="empty" data-testid="research-empty">
              <div>
                Select a research run from the history, or launch a new research
                run by entering a prompt.
              </div>
