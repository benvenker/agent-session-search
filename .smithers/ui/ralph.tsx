...
function extractIterations(events: unknown[]): IterationEvent[] {
  ...
  for (const raw of events) {
    ...
    if (
      kind.indexOf("complete") === -1 &&
      kind.indexOf("finish") === -1 &&
      kind.indexOf("output") === -1
    )
      continue;
...
function App() {
  ...
  const ralphOutput = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "ralph",
    iteration: 0,
  });
  ...
  const latestSummary = useMemo(
    () => extractSummary(ralphOutput.data),
    [ralphOutput.data]
  );
  const iterationCount =
    iterations.length > 0 ? iterations.length : latestSummary ? 1 : 0;
          ...
          <div className="timeline" data-testid="ralph-timeline">
            ...
            {iterations
              ...
              .map((it) => (
                <div
                  className="iter-card"
                  key={it.seq}
                  data-testid="ralph-iteration-event"
                >
                  <div className="row">
            ...
            {!latestSummary && iterations.length === 0 ? (
              <div className="empty" data-testid="ralph-empty">
                ...
                <p>
                  Ralph runs a single task in an infinite loop, posting a fresh
                  summary after every iteration. Enter a prompt and start the
                  loop to watch each iteration land here. Cancel whenever the
                  work is done.
                </p>
                <button
                  className="button primary"
