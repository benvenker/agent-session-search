...
function extractCommits(
  value: unknown
): { summary: string; commits: Commit[] } | null {
  const row = rowOf(value);
  if (!row) return null;
  const summary = asString(row.summary);
  if (summary === undefined) return null;
...
function App() {
  ...
  const statusOut = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "vcs:status",
    iteration: 0,
  });
  const logOut = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "vcs:log",
    iteration: 0,
  });
  const diffOut = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "vcs:diff",
    iteration: 0,
  });
  const messageOut = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "vcs:message",
    iteration: 0,
  });
  const rebaseOut = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "vcs:rebasePlan",
    iteration: 0,
  });
        ...
        <div className="content">
            ...
            <section className="panel" data-testid="vcs-status">
              ...
              {status.changes.map((c) => (
                <div className="change" key={c.path} data-testid="vcs-change">
                  <span className={"glyph " + c.code.slice(0, 1)}>
                    {c.code.slice(0, 1) || "?"}
                  </span>
                  <span className="path">{c.path}</span>
                  <span
                    className={c.staged ? "dot on" : "dot"}
                    title={c.staged ? "staged" : "unstaged"}
                  />
                </div>
              ))}
            </section>
          ...
          {!hasAny ? (
            <div className="empty" data-testid="vcs-empty">
              <div>
                {activeRunId ? "Waiting for the workflow…" : "No VCS runs yet."}
              </div>
              <div className="desc">
                Pick a backend and an action, then Run. <b>status</b> and{" "}
                <b>log</b> read the working tree directly;
                <b> commit</b> has an agent draft a message from the staged
                diff, and <b>rebase-plan</b> has an agent plan the rebase.
                Nothing is executed against the repo.
              </div>
            </div>
          ) : null}
