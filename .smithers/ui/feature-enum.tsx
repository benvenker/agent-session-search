...
function extractInventory(value: unknown): FeatureInventory | null {
  ...
  const groups: FeatureGroup[] = Object.keys(rawGroups).map((name) => {
    ...
    const features = Array.isArray(list)
  ...
  const hasContent =
    ...
    asNumber(row.totalFeatures) !== undefined;
...
function isRefiningExisting(input: unknown): boolean {
  ...
  return (
    input.existingFeatures != null && !Array.isArray(input.existingFeatures)
  );
}
...
function App() {
  ...
  const resultOutput = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "feature-enum:result",
    iteration: 0,
  });
  const scanOutput = useGatewayNodeOutput({
    runId: activeRunId,
    nodeId: "feature-enum:scan",
    iteration: 0,
  });
  ...
  const stages = useMemo(() => {
    ...
    if (!refiningExisting)
      labels.push({
        id: "feature-enum:scan",
        name: "Scan",
        sub: "Initial codebase sweep",
      });
    const refineN = Math.max(1, requestedRefines);
    for (let i = 1; i <= refineN; i += 1) {
      labels.push({
        id: "feature-enum:refine:" + i,
        name: "Refine " + i,
        sub: "Add, decompose, prune",
      });
    }
    labels.push({
      id: "feature-enum:result",
      name: "Result",
      sub: "Final inventory",
    });
    ...
    return labels.map(
      (
        label,
        index
      ): { id: string; name: string; sub: string; state: StageState } => {
  ...
  const visibleGroups = useMemo(() => {
    ...
    return inventory.groups
      .map((g) => {
        ...
        const features = groupMatch
          ? g.features
          : g.features.filter((f) => f.toLowerCase().includes(q));
    ...
    <main className="shell" data-testid="feature-enum-ui">
      ...
      <header className="topbar">
        <div className="title-group">
          ...
          {activeRun ? (
            <span
              ...
              data-testid="feature-enum-status"
            >
        ...
        <div className="toolbar">
          ...
          <button
            className="button"
            data-testid="feature-enum-refresh"
      ...
      <div className="main">
        <aside className="rail" data-testid="feature-enum-runsummary">
          ...
          <div className="meta">
            ...
            <div className="meta-row">
              ...
              <span className="mono">
                {shortHash(inventory?.lastCommitHash)}
              </span>
            </div>
            <div className="meta-row">
        ...
        <div className="content">
          {inventory && inventory.groups.length > 0 ? (
                      ...
                      {!isCollapsed ? (
                        <ul className="feature-list">
                          ...
                          {group.features.length === 0 ? (
                            <li>No features in this group.</li>
                          ) : null}
                        </ul>
                      ) : null}
                    </div>
            ...
            <div className="empty" data-testid="feature-enum-inventory-empty">
              ...
              <div className="hint">
                Create or refine a feature inventory. Set a refine iteration
                count and click Launch Scan to enumerate features grouped by
                domain.
              </div>
              <button
                className="button primary"
        ...
        <aside className="sidebar">
          ...
          <div
            ...
            data-testid="feature-enum-notes"
          >
            ...
            {featureRuns.map((r) => {
              ...
              return (
                <button
                  key={r.runId}
                  className={
                    "run-row" + (r.runId === activeRunId ? " active" : "")
                  }
