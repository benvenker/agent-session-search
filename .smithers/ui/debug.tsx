...
function IterationPanels(props: { runId: string; iteration: number }) {
  ...
  const review0 = useGatewayNodeOutput({
    runId,
    nodeId: "debug:review:0",
    iteration,
  });
  const review1 = useGatewayNodeOutput({
    runId,
    nodeId: "debug:review:1",
    iteration,
  });
  const review2 = useGatewayNodeOutput({
    runId,
  ...
  const reviews = useMemo(
    () =>
      REVIEWER_SLOTS.map((i) => extractReview(reviewData[i])).filter(
          ...
          {implement ? (
            <>
              <div
                ...
                data-testid="debug-implement-summary"
              >
                {implement.summary}
              </div>
              {implement.filesChanged.length > 0 ? (
                ...
                <div
                  className="sub-empty"
                  data-testid="debug-implement-files-empty"
                >
        ...
        <div className="panel" data-testid="debug-validate-panel">
          ...
          {validate ? (
            <>
              <div
                className="panel-summary"
                data-testid="debug-validate-summary"
              >
                {validate.summary}
              </div>
          ...
          {activeRunId ? (
            <>
              ...
              <div className="timeline" data-testid="debug-timeline">
                {Array.from({ length: MAX_ITERATIONS }, (_, i) => i).map(
                  (i) => (
                    <button
                      key={i}
                      className={
            ...
            <div className="empty" data-testid="debug-empty">
              <div>
                No debug run yet. Describe the bug above and start a run.
              </div>
              <button
                className="button primary"
