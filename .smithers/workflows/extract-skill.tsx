...
const inputSchema = z.object({
  runId: z
    ...
    .default(null)
    .describe(
      "Run to harvest from. Null analyses the prompt/context alone, with no run state."
    ),
  prompt: z
    .string()
    .default(
    ...
    .describe(
      "What to harvest, plus any context the analysis should ground itself in."
    ),
});
...
const analyzeSchema = z.looseObject({
  repeatedPattern: z
    .string()
    .describe(
      "One paragraph: the durable, repeatable pattern observed in the run/context."
    ),
  reusableAsSkill: z
    .boolean()
    .describe(
      "True when the pattern is best captured as an agent skill doc the workers can reuse."
    ),
  reusableAsWorkflow: z
    .boolean()
    .describe(
      "True when the pattern is better captured as a whole new Smithers workflow."
    ),
  memoryFacts: z
    .array(z.string())
    .default([])
    .describe(
      "Durable, run-independent facts worth remembering across future runs."
    ),
});
...
const proposeSchema = z.looseObject({
  proposedSkill: z
    .object({
      ...
      description: z
        .string()
        .describe("One line: what it does and when to reach for it."),
      body: z
        .string()
        .describe("Markdown body of the skill doc, ready to write to disk."),
    })
    .nullable()
    .default(null),
  proposedWorkflow: z
    .object({
      ...
      sketch: z
        .string()
        .describe("A short prose sketch of the workflow graph and its stages."),
    })
    .nullable()
    .default(null),
  memoryToPersist: z
