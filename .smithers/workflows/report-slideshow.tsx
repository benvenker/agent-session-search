...
const inputSchema = z.object({
  ...
  title: z
    ...
    .default(null)
    .describe(
      "Optional report title. Null lets the render step derive one from the run."
    ),
});
...
const gatherSchema = z.looseObject({
  ...
  state: z
    .string()
    .default("unknown")
    .describe("Run status: running | completed | failed | unknown."),
  nodes: z
    .array(
      z.looseObject({
        id: z.string().default(""),
  ...
  summary: z
    .string()
    .default("")
    .describe("A short human summary of what the run did and where it ended."),
  raw: z
    .string()
    .default("")
    .describe(
      "The raw inspect JSON (truncated) for the renderer to mine for detail."
    ),
});
...
const renderSchema = z.looseObject({
  ...
  html: z
    .string()
    .describe(
      "A complete, self-contained HTML document (inline CSS, no external deps)."
    ),
  slideCount: z
    .number()
    .default(0)
    .describe("How many slide sections the report contains."),
});
    ...
    <Workflow name="report-slideshow">
      <Sequence>
        ...
        <Task id="gather" output={outputs.gather}>
          {async () => {
            const res =
              await $`bunx smithers-orchestrator inspect ${runId} --format json --full-output`
                .nothrow()
                .quiet();
            ...
            const state = ok
              ? asString(parsed.status ?? parsed.state ?? "unknown") ||
                "unknown"
              : "unknown";
            const summary = ok
        ...
        {gather ? (
          <Task id="render" output={outputs.render} agent={agents.smart}>
            <RenderPrompt runId={runId} title={fallbackTitle} gather={gather} />
          </Task>
        ) : null}
      </Sequence>
    </Workflow>
