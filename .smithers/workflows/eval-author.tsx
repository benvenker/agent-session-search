...
const inputSchema = z.object({
  ...
  workflow: z
    ...
    .default(null)
    .describe(
      "Path or id of the workflow the eval suite targets. Null leaves a placeholder in the run command."
    ),
});
...
const evalCaseSchema = z.looseObject({
  ...
  input: z
    .looseObject({})
    .describe("The workflow input object this case runs with."),
  expected: z
    .looseObject({})
    .describe(
      "Assertion object: status, output (exact), and/or outputContains (partial)."
    ),
  rubric: z
    .string()
    .describe(
      "Plain-English pass/fail criteria for a human or judge reviewing this case."
    ),
});
...
const derivedSuiteSchema = z.looseObject({
  suiteName: z
    .string()
    .describe("Proposed kebab-case suite id (also the fixture filename stem)."),
  cases: z
    .array(evalCaseSchema)
    .default([])
    .describe("Ordered eval cases covering the acceptance criteria."),
});
...
const writtenSuiteSchema = z.looseObject({
  ...
  runCommand: z
    .string()
    .describe("The smithers eval command to run the suite."),
});
