...
const missionFeatureSchema = z.looseObject({
  ...
  workerType: z
    .enum(["implementation", "test", "docs", "research"])
...
function normalizeFeature(feature: any, index: number): any {
  ...
  return {
    ...
    instructions:
      ...
      feature.instructions.length > 0
        ? feature.instructions
        : `Complete ${title}.`,
    files: asStringArray(feature?.files),
    validation: asStringArray(feature?.validation),
    workerType: ["implementation", "test", "docs", "research"].includes(
      feature?.workerType
    )
      ? feature.workerType
      : "implementation",
...
function normalizeMilestones(
  ...
  maxFeaturesPerMilestone: number
): any[] {
    ...
    .map((milestone: any, index: number) => {
      const title =
        typeof milestone?.title === "string" && milestone.title.length > 0
          ...
          : `Milestone ${index + 1}`;
      const features = (
        Array.isArray(milestone?.features) ? milestone.features : []
      )
      ...
      return {
        ...
        features:
          features.length > 0
            ...
            : [
                normalizeFeature(
                  { title, instructions: milestone?.objective ?? title },
                  0
                ),
              ],
      };
...
function featureNeeds(
  ...
  return Object.fromEntries(
    features.map((feature, index) => [
      `feature${index}`,
      featureTaskId(milestoneIndex, feature),
    ])
  );
}
...
function featureDeps(
  features: any[]
): Record<string, typeof missionFeatureResultSchema> {
  return Object.fromEntries(
    features.map((_, index) => [`feature${index}`, outputs.missionFeature])
  );
}
...
function previousMilestoneSummary(ctx: any): string {
  ...
  return [
    ...
    ...validations.map(
      (entry: any) =>
        `Validation: ${entry.passed ? "passed" : "failed"} - ${entry.summary}`
    ),
  ]
    .slice(-8)
    .join("\n");
...
function milestoneIsTerminal(ctx: any, milestoneIndex: number): boolean {
  const revalidation = ctx.outputMaybe("milestoneValidation", {
    nodeId: milestoneRevalidationId(milestoneIndex),
  });
  if (revalidation) return true;
  const validation = ctx.outputMaybe("milestoneValidation", {
    nodeId: milestoneValidationId(milestoneIndex),
  });
...
function renderFeatureWorker(
  ...
  const workerTask = (
    <Task
      ...
      memory={{
        recall: {
          namespace: missionMemory,
          query: `${plan.goal} ${milestone.title} ${feature.title}`,
          topK: 5,
        },
...
function renderMilestone(
  ...
  const integration = ctx.outputMaybe("milestoneIntegration", {
    ...
    <Sequence>
      <Parallel
        maxConcurrency={Math.min(
          ctx.input.maxConcurrency ?? 3,
          features.length
        )}
      >
        {features.map((feature: any) =>
          renderFeatureWorker(ctx, plan, milestone, milestoneIndex, feature)
        )}
      </Parallel>
      <Task
        id={integrationId}
      ...
      >
        {(deps: any) => {
          const results = features.map(
            (_: any, index: number) => deps[`feature${index}`]
          );
          return (
        ...
        <Sequence>
          <Task
            ...
            memory={{
              remember: {
                namespace: missionMemory,
                key: milestoneFollowUpId(milestoneIndex),
              },
            }}
          >
            {(deps: any) => (
          ...
          <Task
            ...
            memory={{
              remember: {
                namespace: missionMemory,
                key: milestoneRevalidationId(milestoneIndex),
              },
            }}
          >
            {(deps: any) => (
...
export default smithers((ctx) => {
  ...
  const approval = ctx.outputMaybe("missionApproval", {
    nodeId: "mission:approve-plan",
  });
  const approvalRequired = ctx.input.requirePlanApproval ?? true;
  const approvalDenied =
    approvalRequired && approval && approval.approved === false;
  const approved = !approvalRequired || approval?.approved === true;
  const milestones = normalizeMilestones(
    plan,
    ctx.input.maxMilestones ?? 6,
    ctx.input.maxFeaturesPerMilestone ?? 6
  );
    ...
    <Workflow name="mission">
      <Sequence>
        <Task
          ...
          heartbeatTimeoutMs={900_000}
          memory={{
            remember: { namespace: missionMemory, key: "mission:plan" },
          }}
        >
          <AskUserInstructions />
          <MissionPlanPrompt
        ...
        {plan && approvalRequired && !approval && (
          <Approval
            ...
            request={{
              ...
              summary:
                plan.summary ||
                "Review the scoped mission plan before workers begin.",
              metadata: {
        ...
        {approvalDenied && (
          <Task id="mission:cancelled" output={outputs.missionFinal}>
            {{
              status: "cancelled",
              summary:
                `Mission plan was not approved. ${approval?.note ?? ""}`.trim(),
        ...
        {plan &&
          approved &&
          activeIndex >= milestones.length &&
          renderFinal(ctx, plan, milestones)}
      </Sequence>
    </Workflow>
  );
});
