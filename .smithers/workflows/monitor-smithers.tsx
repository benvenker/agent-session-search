...
const inputSchema = z.object({
  staleMinutes: z
    ...
    .default(15)
    .describe(
      "A run with no recent activity past this many minutes is treated as stale/stuck."
    ),
});
...
async function pollRuns(staleMinutes: number) {
  const res = await $`bunx smithers-orchestrator ps --format json --all`
    .nothrow()
    .quiet();
  if (res.exitCode !== 0) {
    return {
      runs: [],
      summary: "Could not read runs from `smithers ps` (non-zero exit).",
    };
  }
  ...
  try {
    ...
    return {
      runs: [],
      summary: "Could not parse `smithers ps --format json` output.",
    };
  }
  ...
  const runs = rawList.map((row) => {
    ...
    const lastEvent =
      step != null && String(step) !== "—" ? String(step) : null;
    return { runId, status, ageMinutes, lastEvent };
  });

  const staleCount = runs.filter(
    (run) => run.ageMinutes >= staleMinutes
  ).length;
  return {
    runs,
