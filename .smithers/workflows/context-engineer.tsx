...
const inputSchema = z.object({
  prompt: z
    ...
    .describe(
      "The vague user script the concierge turns into a context contract and then executes."
    ),
  review: z
    .boolean()
    .default(true)
    .describe(
      "Pause for human approval of the context contract before any work is executed."
    ),
});
...
const classifySchema = z.looseObject({
  modes: z
    ...
    .default([])
    .describe(
      "The task modes the script maps onto (e.g. research, planning, implementation, debugging, report)."
    ),
  durable: z
    .boolean()
    .describe(
      "True when the work needs ordering, crash-recovery, approvals, or loops — i.e. a real workflow."
    ),
  reason: z
    .string()
    .default("")
    .describe("One or two sentences justifying the modes + durable call."),
});
...
const contractSchema = z.looseObject({
  goal: z
    .string()
    .describe("One sentence: the outcome the script is really after."),
  nonGoals: z
    .array(z.string())
    .default([])
    .describe("Explicitly out of scope, to stop drift."),
  assumptions: z
    .array(z.string())
    .default([])
    .describe("Reasonable assumptions made to fill gaps in the script."),
  inputs: z
    .array(
      z.looseObject({
        name: z.string(),
  ...
  missingInputs: z
    .array(z.string())
    .default([])
    .describe("Inputs the script is missing — candidates for grilling."),
  availableTools: z
    .array(z.string())
    .default([])
    .describe("Tools/commands available to do the work."),
  availableSkills: z
    .array(z.string())
    .default([])
    .describe("Relevant skills found under .smithers/skills."),
  constraints: z
    .array(z.string())
    .default([])
    .describe("Hard constraints the work must respect."),
  risks: z
    .array(z.string())
    .default([])
    .describe("Risks or side effects to guard against."),
  desiredArtifacts: z
    .array(z.string())
    .default([])
    .describe("Concrete artifacts a successful run should produce."),
  successCriteria: z
    .array(z.string())
    .default([])
    .describe("How we know the work is done."),
  verificationSignals: z
    .array(z.string())
    .default([])
    .describe(
      "Observable signals (tests, traces, reviews) that prove each success criterion."
    ),
});
...
const routeSchema = z.looseObject({
  selectedRoute: z
    ...
    .describe(
      "How the work is carried out: a one-shot task, a set of skills, a durable workflow, or hand to a human."
    ),
  selectedSkills: z
    .array(z.string())
    .default([])
    .describe("Skills the executor should load, best-first."),
  selectedWorkflow: z
    .string()
    .nullable()
    .default(null)
  ...
  durableRequired: z
    .boolean()
    .default(false)
    .describe("True when the route genuinely needs durable execution."),
  humanApprovalRequired: z
    .boolean()
    .default(false)
    .describe("True when a human gate is needed before side effects."),
  reason: z
    .string()
    .default("")
    .describe("Why this route fits the contract and classification."),
});
  ...
  gates: z
    .array(
      z.looseObject({
        criterion: z
          .string()
          .describe("The success criterion this gate enforces."),
        ...
        gateType: z
          ...
          .describe(
            "blocking stops the run; warning flags; informational only records."
          ),
        failureAction: z
          .string()
          .describe("What happens when this gate fails."),
        evidenceRequired: z
          .array(z.string())
          .default([])
          .describe(
            "Concrete artifacts that prove the gate passed (logs, diffs, reports, traces)."
          ),
      })
    )
    .default([])
    .describe(
      "One gate per success criterion; every blocking criterion names a verification method."
    ),
  summary: z
    .string()
    .default("")
    .describe("2-3 sentence overview of the backpressure plan."),
});
...
const executeSchema = z.looseObject({
  summary: z.string().describe("What was done or dispatched this iteration."),
    ...
    .boolean()
    .describe("True when the contracted work is fully carried out."),
  artifacts: z
    .array(z.string())
    .default([])
    .describe("Files written, outputs produced, or workflows dispatched."),
});
...
const reportSchema = z.looseObject({
  html: z
    .string()
    .describe(
      "A complete, self-contained HTML slideshow report (inline CSS, no external deps)."
    ),
  summary: z
    .string()
    .default("")
    .describe("One-line summary of the run for the CLI."),
});
...
export default smithers((ctx) => {
  ...
  const contract = ctx.outputMaybe("inventory", {
    nodeId: "inventory-context",
  });
  const route = ctx.outputMaybe("route", { nodeId: "route" });
  const backpressure = ctx.outputMaybe("backpressure", {
    nodeId: "build-backpressure",
  });
  ...
  return (
    <Workflow name="context-engineer">
      <Sequence>
        ...
        {route ? (
          <Task
            id="build-backpressure"
            output={outputs.backpressure}
            agent={agents.smart}
          >
            <BackpressurePrompt
              prompt={ctx.input.prompt}
        ...
        <Branch
          ...
          then={
            <Approval
              ...
              request={{
                title:
                  `Approve context contract: ${contract?.goal ?? ctx.input.prompt}`.slice(
                    0,
                    120
                  ),
                summary:
