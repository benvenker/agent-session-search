...
const inputSchema = z.object({
  prompt: z
    ...
    .describe(
      "The goal / acceptance criteria to turn into a backpressure gate matrix."
    ),
});
  ...
  gates: z
    .array(
      z.object({
        criterion: z
          .string()
          .describe("The acceptance criterion this gate enforces."),
        ...
        checkedBy: z
          .string()
          .describe(
            "Who/what runs the check (a task id, scorer, human role, or tool)."
          ),
        failureAction: z
          .string()
        ...
        humanApprovalRequired: z
          ...
          .default(false)
          .describe(
            "True if a durable human approval gate is needed for this criterion."
          ),
      })
    )
    .default([])
    .describe(
      "One gate per criterion; every blocking criterion maps to a verification method."
    ),
  summary: z
    .string()
    .default("")
...
export default smithers((ctx) => {
  ...
  const criteria = ctx.outputMaybe("extractCriteria", {
    nodeId: "extract-criteria",
  });
  ...
  return (
    <Workflow name="backpressure-plan">
      <Sequence>
        ...
        <Task
          id="extract-criteria"
          output={outputs.extractCriteria}
          agent={agents.smart}
        >
