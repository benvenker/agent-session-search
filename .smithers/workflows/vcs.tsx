...
function readStatus(tool: string) {
  if (tool === "jj") {
    ...
    const changes = nonEmptyLines(st.out)
      ...
      .filter(
        (line) =>
          !line.startsWith("Working copy") &&
          !line.startsWith("Parent commit") &&
          !line.startsWith("The working copy")
      )
      .map((line) => ({
    ...
    return {
      ...
      summary:
        changes.length === 0
          ? "Working copy clean"
          : changes.length + " change(s) in the working copy",
    };
  }
...
function readLog(tool: string) {
  const repoArgs =
    tool === "jj" ? ["root"] : ["rev-parse", "--is-inside-work-tree"];
  if (!run(tool, repoArgs).ok) {
    return {
      tool,
      isRepo: false,
      commits: [],
      summary: "Not a " + tool + " repository",
    };
  }
  const out =
    tool === "jj"
  ...
  const commits = nonEmptyLines(out).map((line) => {
    ...
    return sp > 0
      ? { id: line.slice(0, sp), subject: line.slice(sp + 1).trim() }
      : { id: line, subject: "" };
  });
  return {
    tool,
    ...
    commits,
    summary:
      commits.length + " recent " + (tool === "jj" ? "change(s)" : "commit(s)"),
  };
}
...
function readDiff(tool: string) {
  if (tool === "jj") {
    ...
    return {
      ...
      isRepo: true,
      files: [],
      patch: d.out.slice(0, 8000),
      truncated: d.out.length > 8000,
    };
  }
  if (!run("git", ["rev-parse", "--is-inside-work-tree"]).ok)
    return { tool, isRepo: false, files: [], patch: "", truncated: false };
...
export default smithers((ctx) => {
  ...
  if (action === "commit") {
    const diff = ctx.outputMaybe("diff", { nodeId: "vcs:diff" }) as
      | { patch?: string }
      | undefined;
    const patch =
