...
const inputSchema = z.object({
  prompt: z
    ...
    .describe(
      "Plain-English description of the task to route — run directly or hand to a durable workflow."
    ),
});
...
const classifySchema = z.looseObject({
  ...
  durable: z
    .boolean()
    .describe(
      "True when the task needs ordering, crash-recovery, approvals, or loops — i.e. a real workflow."
    ),
...
const executeSchema = z.looseObject({
  summary: z.string().describe("What was done, in a sentence or two."),
    ...
    .boolean()
    .describe("True when the task was fully completed in this single step."),
});
...
const recommendSchema = z.looseObject({
  ...
  why: z
    .string()
    .describe(
      "Why this workflow fits the task — what durable behaviour it provides."
    ),
