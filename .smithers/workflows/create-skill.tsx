...
const inputSchema = z.object({
  prompt: z
    ...
    .describe(
      "Plain-English description of the agent skill you want Smithers to author."
    ),
  name: z
    .string()
    .nullable()
    .default(null)
    .describe(
      "Desired kebab-case skill id. Null lets the clarify/design steps choose one."
    ),
  review: z
    .boolean()
    .default(true)
...
const skillSpecSchema = z.looseObject({
  ...
  purpose: z
    .string()
    .describe("One sentence: what the skill equips an agent to do."),
  whenToUse: z
    .string()
  ...
  capabilities: z
    ...
    .default([])
    .describe(
      "The concrete things an agent can do once this skill is in context."
    ),
  inputs: z
    .array(
...
const designSchema = z.looseObject({
  ...
  frontmatter: z
    .object({
      ...
      description: z
        .string()
        .describe(
          "One line: what it does and when to use it — goes in YAML frontmatter."
        ),
    })
  ...
  supportingFiles: z
    ...
    .default([])
    .describe(
      "Optional extra files (scripts, references) the skill ships alongside SKILL.md."
    ),
...
const { Workflow, Task, Sequence, Branch, Approval, smithers, outputs } =
  createSmithers({
    input: inputSchema,
