...
const issueSchema = z.object({
  check: z
    .string()
    .describe("Stable id of the check that produced this issue."),
...
const checkSchema = z.looseObject({
  ...
  score: z
    .number()
    .describe("0–100 health score; 100 means every check passed."),
});
...
const adviseSchema = z.looseObject({
  fixes: z
    .array(z.string())
    .default([])
    .describe("Concrete suggestions, one per non-ok issue."),
  summary: z.string(),
});
...
function diagnose(raw: string): z.infer<typeof checkSchema> {
    ...
    return {
      issues: [
        fail(
          "parse",
          "error",
          "Contract must be a JSON object at the top level."
        ),
      ],
  ...
  if (isNonEmptyString(contract.goal)) {
    ...
    issues.push(
      fail(
        "hasGoal",
        "error",
        "Missing a non-empty `goal` describing the intended outcome."
      )
    );
  }
  ...
  if (isNonEmptyString(outputSpec) || isObject(outputSpec)) {
    ...
    issues.push(
      fail(
        "hasOutputSpec",
        "error",
        "Missing an `outputSpec` describing the produced artifact."
      )
    );
  }
  ...
  const acceptanceCriteria = asArray(
    contract.acceptanceCriteria ?? contract.acceptance_criteria
  );
  if (acceptanceCriteria.length > 0) {
    issues.push(
      ok(
        "hasAcceptanceCriteria",
        acceptanceCriteria.length + " acceptance criteria declared."
      )
    );
  } else {
    issues.push(
      fail(
        "hasAcceptanceCriteria",
        "error",
        "Missing `acceptanceCriteria`; no way to know when the work is done."
      )
    );
  }

  // allBlockingCriteriaHaveVerification — every blocking criterion names a verification.
  const blocking = acceptanceCriteria.filter(
    (c) => isObject(c) && c.blocking === true
  );
  if (blocking.length === 0) {
    issues.push(
      ok(
        "allBlockingCriteriaHaveVerification",
        "No blocking acceptance criteria to verify."
      )
    );
  } else {
    const unverified = blocking.filter(
      (c) =>
        isObject(c) &&
        !isNonEmptyString(c.verification) &&
        !isNonEmptyString(c.verify)
    );
    if (unverified.length === 0) {
      issues.push(
        ok(
          "allBlockingCriteriaHaveVerification",
          "Every blocking criterion names a verification."
        )
      );
    } else {
      issues.push(
        fail(
          ...
          "error",
          unverified.length +
            " blocking criterion/criteria lack a `verification` step."
        )
      );
    }
  }
  ...
  const requiredInputs = inputs.filter(
    (i) => isObject(i) && i.required === true
  );
  if (requiredInputs.length === 0) {
    issues.push(
      ok("allRequiredInputsHaveSource", "No required inputs to source.")
    );
  } else {
    const sourceless = requiredInputs.filter(
      (i) => isObject(i) && !isNonEmptyString(i.source)
    );
    if (sourceless.length === 0) {
      issues.push(
        ok(
          "allRequiredInputsHaveSource",
          "Every required input declares a `source`."
        )
      );
    } else {
      issues.push(
    ...
    if (unguarded.length === 0) {
      issues.push(
        ok(
          ...
          "Every side effect is gated by an approval."
        )
      );
    } else {
      issues.push(
  ...
  const reportSpec =
    contract.reportSpec ?? contract.report ?? contract.report_spec;
  if (isNonEmptyString(reportSpec) || isObject(reportSpec)) {
    ...
    issues.push(
      fail(
        "reportSpecExists",
        "info",
        "No `reportSpec`; results have no declared reporting format."
      )
    );
  }
  ...
  const summary =
    errors > 0
      ? errors +
        " error(s) and " +
        warnings +
        " warning(s) — contract is incomplete (score " +
        score +
        "/100)."
      : warnings > 0
        ? warnings +
          " warning(s) — contract is usable but could be tightened (score " +
          score +
          "/100)."
