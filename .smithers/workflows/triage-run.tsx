...
const gatherSchema = z.looseObject({
  state: z
    .string()
    .describe(
      "The run's overall status (running | paused | failed | completed | unknown)."
    ),
  failingNodes: z
...
const diagnoseSchema = z.looseObject({
  ...
  evidence: z
    ...
    .default([])
    .describe(
      "Concrete observations from state/events that support the hypothesis."
    ),
  confidence: z
...
const recommendSchema = z.looseObject({
  recommendedAction: z
    ...
    .describe(
      "fix code, rewind to an earlier frame, retry the failing task, or escalate to a human."
    ),
  command: z
    .string()
    .describe(
      "The exact CLI command to run next (e.g. a smithers rewind / retry-task invocation)."
    ),
  rationale: z
    .string()
    .describe(
      "Why this action over the alternatives, grounded in the diagnosis."
    ),
});
    ...
    <Workflow name="triage-run">
      <Sequence>
        ...
        <Task id="gather" output={outputs.gather}>
          {async () => {
            const inspectRes =
              await $`bunx smithers-orchestrator inspect ${runId} --json`
                .nothrow()
                .quiet();
            const eventsRes =
              await $`bunx smithers-orchestrator events ${runId}`
                .nothrow()
                .quiet();
